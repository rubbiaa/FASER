...
// extract pseudorapidity of incoming neutrino based on vertex position
// and direction from ATLAS IP to neutrino interaction vertex in 3DCAL and AHCAL
// ATLAS IP is 480 m = 480 000 mm, upstream of FASERCAL neutrino interaction point
//
// for each neutrino interaction vertex in FASERCAL region (vtx, vty, vtz), the direction vector from
// ATLAS IP to the vertex is ()dir = (vtx, vty, vtz + 480000 mm)
//
// Then: Transverse momentum pt = sqrt(dir_x^2 + dir_y^2)
//       Longitudinal momentum pz = dir_z
//       theta = atan2(pt, pz)
//       eta = -log(tan(theta/2))
//
// Usage:
//   etaCoverage_geometry <genieroot> <outputRootFile>
// ------------------------------------------------------------
...
// ATLAS IP position relative to detector (assume 480 m upstream from 3Dcal center)
constexpr double Z_IP_MM              = -480000.0;  
...
int main(int argc, char **argv)
{
    ...
    for (Long64_t ievt = 0; ievt < n_entries; ++ievt)
    {
        ...
        double dir_x = x_mm;
        double dir_y = y_mm;
        double dir_z = z_mm - Z_IP_MM; // z relative to IP
        //double dir_z = 480000.0; // assume neutrino comes from IP at (0,0,-480000 mm)
        double pt = std::sqrt(dir_x*dir_x + dir_y*dir_y);
        double theta = std::atan2(pt, dir_z); // angle wrt z-axis
    ...
    save1D(h_eta_3dcal,    "eta_3dcal_geom.png");
    //save1D(h_eta_rearecal, "eta_rearecal.png");
    save1D(h_eta_ahcal,    "eta_ahcal_geom.png");
    ...
    save2D(h_E_vs_eta_3dcal, "E_vs_eta_3dcal_geom.png");
    save2D(h_E_vs_eta_ahcal, "E_vs_eta_ahcal_geom.png");
