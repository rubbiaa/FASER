...
namespace display
{
  ...
  void FaserCalDisplay::GetEventDisplay()
  {
    ...
    int posy = 20;
        fNumberEntryRun = new TGNumberEntryField(fGroupFrame2, -1, 500, TGNumberFormat::kNESInteger);
        ...
        for ( size_t i = 0; i < nhits; i++)
          {
            ...
            if(hittype == 0)
              {
                //std::cout << " voxel hit " << track->fPDG << " " << track->fparentID << " " << track->fprimaryID << " " << track->ftrackID << ""<< fCharmParentID << " " << fTauParentID << std::endl;
