...
void TKinFitter::addMeasParticle( TAbsFitParticle* particle ) {
  ...
  if ( particle != 0 ) {
    ...
    std::cout << "Measured particle points to NULL. It will not be added to the KinFitter." << std::endl;
  }
...
void TKinFitter::addUnmeasParticle( TAbsFitParticle* particle ) {
  ...
  if ( particle != 0 ) {
    ...
    std::cout << "Unmeasured particle points to NULL. It will not be added to the KinFitter." << std::endl;
  }
...
void TKinFitter::setCovMatrix( TMatrixD &V ) {
  ...
  if ( (V.GetNrows() != _nParB) || (V.GetNcols() != _nParB) ) {
    std::cout 
      << "Covariance matrix of measured particles needs to be a " << _nParB << "x" << _nParB << " matrix." << std::endl;
  } else {
    _V.ResizeTo( V );
    _V = V;
  }
...
Bool_t TKinFitter::calcV() {
  ...
  try {
    ...
    std::cout << "Failed to invert covariance matrix V. Fit will be aborted." << std::endl;
    _status = -10;
  }
...
Bool_t TKinFitter::calcVB() {
  ...
  try {
    ...
    std::cout << "Failed to invert matrix VB. Fit will be aborted." << std::endl;
    _status = -10;
  }
...
Bool_t TKinFitter::calcVA() {
  ...
  try {
    ...
    std::cout << "Failed to invert matrix VA. Fit will be aborted." << std::endl;
    _status = -10;
  }
...
void TKinFitter::printMatrix(const TMatrixD &matrix, const TString name) {
  ...
  if (!matrix.IsValid()) {
    std::cout << "Matrix " << name << " is invalid." << std::endl;
    return;
  }
